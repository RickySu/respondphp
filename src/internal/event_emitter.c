...
void rp_event_emitter_on(event_hook_t *event_hook, zend_string *event, zval *hook)
{
    ...
    if((array = zend_hash_find(Z_ARRVAL_P(&event_hook->hook), event)) == NULL) {
        array_init(&new_array);
        add_assoc_zval_ex(&event_hook->hook, event->val, event->len, &new_array);
        array = &new_array;
    }
    ...
    if((array = zend_hash_find(&event_hook->hook_cache, event)) == NULL) {
...
{
    zend_hash_del(Z_ARRVAL_P(&event_hook->hook), event);
    zend_hash_del(&event_hook->hook_cache, event);
}

zval *rp_event_emitter_getListeners(event_hook_t *event_hook, zend_string *event)
{
    return zend_hash_find(Z_ARRVAL_P(&event_hook->hook), event);
}

void rp_event_emitter_emit(event_hook_t *event_hook, zend_string *event, zval *arg)
{
