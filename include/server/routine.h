...
TRAIT_PHP_METHOD(respond_server_routine, event_emitter);
TRAIT_FUNCTION_ARG_INFO(respond_server_routine, event_emitter);
DECLARE_FUNCTION_ENTRY(respond_server_routine) =
{
    PHP_ME(respond_server_routine, __construct, NULL, ZEND_ACC_PUBLIC|ZEND_ACC_CTOR)
    PHP_ME(respond_server_routine, execute, ARGINFO(respond_server_routine, execute), ZEND_ACC_PUBLIC)
    TRAIT_FUNCTION_ENTRY_ME(respond_server_routine, event_emitter)
    PHP_FE_END
};
#endif
